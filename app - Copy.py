...
# Note: folder is 'static' (lowercase) to match your recent change
app = Flask(__name__, static_folder='static', static_url_path='/static')

# ---- 2. Babel Localization Setup (Flask-Babel 3.0+ Style) ----

def get_locale():
    """Logic to select the language."""
...
# Initialize Babel with the locale_selector callback
babel = Babel(app, locale_selector=get_locale)

@app.context_processor
def inject_conf_var():
    """Makes get_locale available inside all Jinja2 templates."""
    return dict(get_locale=get_locale)

# --- 3. Logging Handler Class ---
...
# ---- 6. Helper for Legacy Static Serving (If needed) ----
def _serve_static(filename):
    # This is useful if you have files outside the standard static folder
...
@app.route('/prizes')
def prizes_page():
    """Serves the Prize Management UI."""
    return render_template('prizes.html')

@app.route('/redeem')
def redeem_page():
    """Serves the Point Redemption UI."""
    return render_template('redeem.html')
    
@app.route('/audit_logs')
def audit_logs_page():
    """Serves the Audit Log UI."""
...
def api_add_student():
    ...
    # Pull secure staff identity from the session
    ...
    # 1. Call the Postgres-compatible registration function
    success, message = add_student.add_new_student(student_data)
    
    if success:
        # 2. Record the administrative action in the Audit Log
...
def api_check_duplicates():
    ...
    matches = []
    for s in results:
        # Check if result is a dict (standard) or tuple (fallback)
...
def api_record_transaction():
    """Records points for an activity with a secure staff audit trail."""
    ...
    # 2. Pull identity from secure server-side session
    # This prevents the client from spoofing the 'recorded_by' field.
    staff_identity = session.get('username', 'system')

    # 3. Pass directly to the strongly typed transaction manager
    ...
    try:
        ...
        for row in rows:
            ...
            history.append({
                ...
                "recorded_by": row['recorded_by'] if is_dict else row[5] # New Column
            })
...
def api_create_activity():
    ...
    try:
        cur = conn.cursor()
        # This handles both new entries and updates based on Name
        cur.execute("""
...
# ---- Activities List for the Record Activity page ----
...
def api_list_activities():
    ...
    try:
        cur = conn.cursor()
        # We only need active activities for the recording dropdown
        cur.execute("SELECT id, name, default_points FROM activities WHERE active = TRUE ORDER BY name")
        rows = cur.fetchall()
        
        # Mapping to the format the dropdown expects
        activities = []
        for row in rows:
            is_dict = isinstance(row, dict)
            activities.append({
...
# ---- Activities List for the Activity management page ----
...
def api_list_all_activities():
    ...
    if not conn:
        return jsonify({"success": False, "message": "DB Connection Error"}), 500
    try:
        # Use a dictionary cursor if your db_utils supports it, 
        # but let's write the mapping to be safe for both.
        cur = conn.cursor()
        ...
        activities = []
        for row in rows:
            # We use .get() or dict keys because your log shows 'KeyError: 0'
            # which proves 'row' is a dictionary object.
            activities.append({
...
def api_delete_activity(activity_id):
    ...
    try:
        cur = conn.cursor()
        # 1. Get the name of the activity to check logs
        ...
        # 2. Check if this activity name exists in activity_log
...
def api_add_prize():
    """Creates or updates a prize with a secure audit trail."""
    data = request.get_json() or {}
    name = data.get('name')
    ...
    try:
        ...
        # 2. Record the system change in the Audit Log
        transaction_manager.log_audit_event(
...
def api_list_prizes():
    """Returns all prizes for management or redemption."""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
...
def api_redeem_prize():
    """Handles prize redemption with atomic stock updates and audit tracking."""
    ...
    try:
        ...
        # 4. Record as negative points with secure audit trail
        success, msg = transaction_manager.add_points(
            student_id=student_id, 
            points=-abs(p_cost), 
...
def api_delete_prize(prize_id):
    ...
    try:
        ...
        # 2. Check if this prize name exists in activity_log (Redemptions use "Redemption: Name")
...
def api_view_audit_logs():
    ...
    try:
        cur = conn.cursor()
        # Querying the actual columns: action_type, recorded_by, etc.
        cur.execute("""
        ...
        for row in rows:
            ...
            audit_history.append({
                "id": row['id'] if is_dict else row[0],
                "action": row['action_type'] if is_dict else row[1], # Map to 'action' for JS
                "details": row['details'] if is_dict else row[2],
                "user": row['recorded_by'] if is_dict else row[3],   # Map to 'user' for JS
                "timestamp": row['event_time'] if is_dict else row[4] # Map to 'timestamp' for JS
            })
